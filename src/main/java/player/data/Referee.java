...
// Refree must be aware of the all the game details and data
...
import static player.data.Order.*;
import static player.data.Status.WIN;
import static player.data.Who.COMPUTER;
import static player.data.Who.USER;
...
public class Referee {
    ...
    public Function<String , String> handleOutcome  = nextOrder -> {
        ...
        //  Try in
        Map<Who ,Player> mapping = Map.of(USER, player1 , COMPUTER, player2);
        //IMPROVE
    ...
    public Predicate<List<String>> handEndOfTheGame = list ->{
        ...
        if (player1.getCardLists().size() == 0 || player2.getCardLists().size() == 0) {
            ...
            if (player1.getCardLists().size() == 0) player1.setStatus(WIN);
            if (player2.getCardLists().size() == 0) player2.setStatus(WIN);
            return true;
        }
        else return false;
    };
